...
class MyBSDF(mi.BSDF):
    ...
    def __init__(self, props):
        ...
        # Load the BRDF data for this material
        self.measured_brdf = MeasuredBRDF(self.filename).get_array()
        self.measured_brdf = torch.from_numpy(self.measured_brdf).float().to(device)
    ...
    def half_diff_look_up_brdf(self, theta_h, theta_d, phi_d):
        ...
        # Use the instance-specific measured BRDF data
        return torch.clip(self.measured_brdf[id_theta_h, id_theta_d, id_phi_d, :], 0, None)
    

    def sample(self, ctx, si, sample1, sample2, active=True):
        # Compute the cosine of the incident direction (wi)
        cos_theta_i = mi.Frame3f.cos_theta(si.wi)
        active &= cos_theta_i > 0 

        bs = mi.BSDFSample3f()

        microfacet_distribution = mi.MicrofacetDistribution(mi.MicrofacetType.GGX, self.alpha, sample_visible=True)

        m, bs.pdf = microfacet_distribution.sample(si.wi, sample2)
        ...
        theta_h, theta_d, phi_d = std_to_half_diff(si.wi, bs.wo)

        value = self.half_diff_look_up_brdf(theta_h, theta_d, phi_d)

        value = mi.Vector3f(value[..., 0], value[..., 1], value[..., 2])

        result = value / bs.pdf * cos_theta_o

        return bs, dr.select(active & (bs.pdf > 0.0), result, mi.Vector3f(0))
    ...
    def pdf(self, ctx, si, wo, active=True):
        ...
        pdf_value = microfacet_distribution.pdf(si.wi, m)

        return dr.select((cos_theta_i > 0.0) & (cos_theta_o > 0.0), pdf_value, 0.0)
...
if __name__ == "__main__":
    ...
    for _ in tqdm(range(spp // SPP)):
        image_accumulation.append(mi.render(scene, spp=SPP, seed=seed))
        seed += 1

    image_sum = np.sum(image_accumulation, axis=0)
    image = image_sum / (spp // SPP)


    mi.util.write_bitmap("cuda_measured2.exr", image)
    end_time = time.time()
